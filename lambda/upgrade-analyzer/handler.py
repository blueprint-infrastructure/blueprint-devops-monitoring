...
CHAIN_UPGRADE_CONTEXT = {
    "avalanche": (
        ...
        "  The installer downloads the binary to ~/avalanche-node/ and restarts the service. "
        "  WARNING: If systemd ExecStart points to /usr/local/bin/avalanchego (not ~/avalanche-node/), the installer update will NOT take effect. "
        "  In that case, after running the installer, also copy the binary: sudo cp /home/ubuntu/avalanche-node/avalanchego /usr/local/bin/avalanchego && sudo systemctl restart avalanchego. "
        "  The upgrade_steps MUST include: 1) check ExecStart path, 2) run installer, 3) if needed copy binary to ExecStart path, 4) restart service. "
