...
CHAIN_UPGRADE_CONTEXT = {
    "avalanche": (
        "Service: avalanchego (systemctl). Binary: /usr/local/bin/avalanchego. Config: /etc/avalanche/config.json. "
        "Installed via APT (downloads.avax.network repo). Data dir: check config for data-dir, usually /home/ubuntu/.avalanchego. "
        "No custom subnet VMs — plugins directory is empty. subnet-evm is bundled into avalanchego since 1.14.2. "
        "Do NOT warn about plugin version bumps or rpcchainvm changes — irrelevant for this fleet. "
        "Upgrade method: Use avax-upgrade.sh from bpte repo. Fully automated, runs as root via SSM. "
        "  Script: blueprint-infrastructure/bpte repo, path deploy/avalanche/avax-upgrade.sh (PRIVATE repo). "
        "  The script handles: pre-flight checks → backup → apt install → restart → health wait (600s) → verify. Auto-rollback on failure. "
        "  upgrade_steps should be EXACTLY 2 steps, nothing else: "
        "    1) curl -sL -H 'Authorization: token <GITHUB_TOKEN>' -H 'Accept: application/vnd.github.v3.raw' "
        "       'https://api.github.com/repos/blueprint-infrastructure/bpte/contents/deploy/avalanche/avax-upgrade.sh' -o /opt/avax-upgrade.sh && chmod +x /opt/avax-upgrade.sh "
        "    2) sudo bash /opt/avax-upgrade.sh --target-version <VERSION> (no v prefix, e.g. 1.14.2). MUST use sudo — script requires root. "
        "  Do NOT add ANY other manual steps — no stop, start, copy, backup, config edit. The script handles EVERYTHING including pre-flight, backup, install, restart, health wait, and auto-rollback. "
        "  For multiple nodes: upgrade one at a time, verify health between each. List ALL affected nodes. "
        "  Rollback: the script auto-rolls back on failure. If manual rollback needed: sudo apt-get install -y avalanchego=<OLD_VERSION> && sudo systemctl restart avalanchego. "
        "  Logs: /var/log/avax-upgrade.log and journalctl -u avalanchego. "
        ...
        "  df -h $(grep -oP '\"data-dir\"\\s*:\\s*\"\\K[^\"]+' /etc/avalanche/config.json 2>/dev/null || echo /home/ubuntu/.avalanchego); "
        "  grep -E 'benchlist-fail-threshold|benchlist-min-failing-duration' /etc/avalanche/config.json 2>/dev/null || echo 'No deprecated flags'. "
