...
CHAIN_UPGRADE_CONTEXT = {
    "avalanche": (
        ...
        "  upgrade_steps format: The FIRST step MUST be 'su ubuntu' to switch to ubuntu user. ALL subsequent steps run as ubuntu. "
        "  Use sudo only for systemctl and cp to /usr/local/bin. Do NOT use 'sudo -u ubuntu bash -c' — just switch user first with su ubuntu. "
        "  Exact upgrade sequence after su ubuntu: "
        "    1) cd ~ && wget -q <installer_url> && chmod +x avalanchego-installer.sh && ./avalanchego-installer.sh --version v<VERSION>, "
        "    2) sudo systemctl stop avalanchego, "
        "    3) sudo cp ~/avalanche-node/avalanchego /usr/local/bin/avalanchego, "
        "    4) sudo systemctl start avalanchego. "
        "  Cannot cp while service runs ('Text file busy'), must stop first. "
