...
CHAIN_UPGRADE_CONTEXT = {
    "avalanche": (
        ...
        "Upgrade method: Use the internal avax-upgrade.sh script from bpte repo. "
        "  The script is at https://raw.githubusercontent.com/blueprint-infrastructure/bpte/main/deploy/avalanche/avax-upgrade.sh. "
        "  It uses APT to install, runs as root, has automatic rollback on failure, and waits for health checks. "
        "  upgrade_steps: "
        "    1) Download the script: wget -q https://raw.githubusercontent.com/blueprint-infrastructure/bpte/main/deploy/avalanche/avax-upgrade.sh -O /opt/avax-upgrade.sh && chmod +x /opt/avax-upgrade.sh, "
        "    2) Run with target version (no v prefix): sudo bash /opt/avax-upgrade.sh --target-version <VERSION> (e.g. 1.14.2, NOT v1.14.2). "
        "  The script handles: version check → backup → apt install → restart → health wait → verify. Auto-rollback on failure. "
        "  Do NOT manually stop/start the service or copy binaries — the script handles everything. "
