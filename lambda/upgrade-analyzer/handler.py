...
CHAIN_UPGRADE_CONTEXT = {
    "avalanche": (
        ...
        "  upgrade_steps should be EXACTLY 1 step: "
        "    sudo bash /opt/avax-upgrade.sh --target-version <VERSION> (no v prefix, e.g. 1.14.2). MUST use sudo — script requires root. "
        "  The script is pre-deployed at /opt/avax-upgrade.sh on all nodes. "
        "  Do NOT add ANY other steps — no download, stop, start, copy, backup, config edit. The script handles EVERYTHING including pre-flight, backup, install, restart, health wait, and auto-rollback. "
