...
CHAIN_UPGRADE_CONTEXT = {
    "avalanche": (
        ...
        "  upgrade_steps should be EXACTLY (2 steps only): "
        "    1) Download from private repo using GitHub API: "
        "       curl -sL -H 'Authorization: token <GITHUB_TOKEN>' -H 'Accept: application/vnd.github.v3.raw' "
        ...
        "    2) bash /opt/avax-upgrade.sh --target-version <VERSION> (no v prefix, e.g. 1.14.2) "
        "  Do NOT add manual stop/start/copy steps — the script does everything. "
        "  For multiple nodes: upgrade one at a time, verify health between each. "
